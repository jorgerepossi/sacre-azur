...
export default function PricePreview({ control }: PricePreviewProps) {
  ...
  return (
    <Card className="p-4 bg-muted/30">
      <p className="text-sm font-semibold mb-2">Precios finales:</p>
      <div className="space-y-1 text-sm">
        <div className="flex justify-between">
          <span className="text-muted-foreground">2.5ml:</span>
          <span className="font-mono font-semibold">${formatPrice(calculatePrice(2.5))}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-muted-foreground">5ml:</span>
          <span className="font-mono font-semibold">${formatPrice(calculatePrice(5))}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-muted-foreground">10ml:</span>
          <span className="font-mono font-semibold">${formatPrice(calculatePrice(10))}</span>
        </div>
      </div>
    </Card>
  );

...
const PerfumeListItem = ({item}: PerfumeListItemProps) => {
    ...
    return (
        <TableRow>
            ...
            <TableCell className="font-medium">
                <p className={'text-body-medium'}>${formatNumberWithDots(item.price)}</p>
            </TableCell>
            <TableCell className="font-medium">
                <p className={'text-body-medium text-muted-foreground'}>{item.profit_margin}%</p>
            </TableCell>
            <TableCell className="font-medium">
                <code>{item.external_link}</code>
            </TableCell>
            <TableCell className="font-medium text-right">
                <Flex className={'w-[24px] h-[24px] border justify-center items-center rounded-full overflow-hidden bg-neutral-50    '} >{item.in_stock && <span className={'bg-green-200 w-[24px] h-[24px]'} />}</Flex>
            </TableCell>
        </TableRow>
    );
};

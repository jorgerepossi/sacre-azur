...
export default function DeletePoliciesButton() {
  const deletePolicies = useMutation({
    mutationFn: async () => {
      ...
      const { error: policyError } = await supabase.rpc("delete_all_policies", {
        table_name: "perfume",
      });
      if (policyError)
        throw new Error("Error eliminando policies: " + policyError.message);

      return "Policies eliminadas y RLS desactivado";
    },
    onSuccess: () => {
      alert(
        "Todas las policies de seguridad fueron eliminadas de la tabla 'perfume'. Intenta insertar nuevamente.",
      );
    },
    onError: (error) => {
  ...
  return (
    <Button
      onClick={() => deletePolicies.mutate()}
      disabled={deletePolicies.isPending}
    >
      {deletePolicies.isPending
        ? "Borrando Policies..."
        : "Eliminar Policies de Perfume"}
    </Button>
  );
}

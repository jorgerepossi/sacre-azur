...
import { BarChart, LineChart, ArrowUpRight, ArrowDownRight, Calendar } from "lucide-react"
...
export default function SalesOverview() {
    ...
    const totalSales = salesData[timeRange as keyof typeof salesData].reduce((sum, item) => sum + item.sales, 0)


    const totalProductGrowth = productGrowthData.reduce((sum, item) => sum + item.growth, 0)
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Card>
                    ...
                    <CardContent>
                        ...
                        <p className="text-xs text-muted-foreground">+{Math.round(totalSales * 0.15)} desde el período anterior</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{productGrowthData.reduce((sum, item) => sum + item.count, 0)}</div>
                ...
                <TabsContent value="sales" className="space-y-4">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Visualización de ventas durante el período seleccionado</CardDescription>
                        </CardHeader>
                        <CardContent className="pl-2">
                            <div className="h-[300px]">
                                <div className="flex h-full items-end gap-2">
                                    {salesData[timeRange as keyof typeof salesData].map((item) => (
                                        <div key={item.name} className="relative flex flex-1 flex-col items-center">
                                            <div
                                            ...
                                            <span className="absolute -top-7 text-xs font-medium">{item.sales}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Crecimiento de productos por categoría</CardDescription>
                            </CardHeader>
                            <CardContent>
                                    ...
                                    {productGrowthData.map((category) => (
                                        <div key={category.name} className="flex items-center">
                                            ...
                                            <div className="w-[30%] flex items-center">
                                                ...
                                                <span className={`ml-2 text-xs ${category.growth > 0 ? "text-green-500" : "text-red-500"}`}>
                          {category.growth > 0 ? "+" : ""}
                                                    {category.growth}
                        </span>
                                            </div>
                        ...
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Los perfumes con mayor número de ventas</CardDescription>
                            </CardHeader>
                            <CardContent>
                                <div className="space-y-4">
                                    {topSellingProducts.map((product) => (
                                        <div key={product.name} className="flex items-center">
                                            <div className="w-[60%] font-medium truncate" title={product.name}>
                                                {product.name}
                                            </div>
                                            <div className="w-[40%] flex items-center">
                                                <div className="w-full h-2 rounded-full bg-muted mr-3">
                                                    <div
